...
//                    ManegerPhongThueFragment.datPhongAdapter.notifyDataSetChanged();
    ...
    //Cập nhật Phòng
    public void updatePhong(final GiaoDich giaoDich) {
        reference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot data : dataSnapshot.getChildren()) {
                    if (data.child("Phong").child("idPhong").getValue(String.class).equalsIgnoreCase(giaoDich.getChiTietPhong().getIdPhong())) {
                        key = data.getKey();
